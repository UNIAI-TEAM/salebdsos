...
function ProfilePage() {
  ...
  return <div className="space-y-5"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3"><div className="min-w-0"><h1 className="truncate text-2xl font-semibold">Profile của tôi</h1><p className="max-w-[18rem] text-sm text-muted-foreground sm:max-w-none">Hồ sơ chi tiết khách hàng nhìn thấy sau khi quét QR.</p></div><div className="flex shrink-0 gap-2"><Link to="/digital-card" search={{}} aria-label="Mở danh thiếp" title="Mở danh thiếp" className="grid h-10 w-10 place-items-center rounded-lg border border-border"><IdCard className="h-4 w-4" /></Link><Link to="/digital-card/edit" aria-label="Sửa danh thiếp" title="Sửa danh thiếp" className="grid h-10 w-10 place-items-center rounded-lg border border-border"><Pencil className="h-4 w-4" /></Link></div></div><ProfilePresentation card={cardQ.data} projects={projects} publicUrl={publicUrl} /></div>;
}
